...
describe('cyclePct edge cases', () => {
	it('returns 0 when pool is 0 (no credits anywhere)', () => {
		const u: Usage = {planId: '', monthlyCredits: 0, purchasedCredits: 0, freeCredits: 0, totalSpent: 50, fiveHourUsed: 0, fiveHourCap: 0, weeklyUsed: 0, weeklyCap: 0};
		expect(cyclePct(u)).toBe(0);
	});
	it('returns 0 when totalSpent is negative (bad API data)', () => {
		const u: Usage = {planId: 'individual-pro', monthlyCredits: 30, purchasedCredits: 0, freeCredits: 0, totalSpent: -5, fiveHourUsed: 0, fiveHourCap: 0, weeklyUsed: 0, weeklyCap: 0};
		expect(cyclePct(u)).toBe(0);
	});
	it('caps at 100 even with huge overspend', () => {
		const u: Usage = {planId: 'individual-go', monthlyCredits: 10, purchasedCredits: 0, freeCredits: 0, totalSpent: 999999, fiveHourUsed: 0, fiveHourCap: 0, weeklyUsed: 0, weeklyCap: 0};
		expect(cyclePct(u)).toBe(100);
	});
	it('includes purchased + free credits in the pool', () => {
		const u: Usage = {planId: 'individual-go', monthlyCredits: 10, purchasedCredits: 40, freeCredits: 0, totalSpent: 25, fiveHourUsed: 0, fiveHourCap: 0, weeklyUsed: 0, weeklyCap: 0};
